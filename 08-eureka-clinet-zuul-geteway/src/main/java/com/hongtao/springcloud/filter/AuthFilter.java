...
public class AuthFilter extends ZuulFilter {
    //当前方法的返回值用于决定当前过滤器的类型
    public String filterType() {
        //返回pre表示当前过滤器为前置过滤器，需要在执行转发（访问服务提供者）前执行，通常用于做身份认证
        return "pre";
    }

    //过滤器的序号，如果有多个同类型的过滤那么根据这个返回值大小决定执行先后顺序
    public int filterOrder() {
        return 0;
    }
    //当前过滤器是否启动，返回true表示启用当前过滤器false则表示不启用
...
//        获取请求中的请求参数 token（身份令牌用于验证请求身份是否合法）
        ...
        if (token==null || !"123".equals(token)){
            ...
            System.out.println("请求合法继续执行请求准备进入服务器或下一个过滤器");
        }
        return null;
    }
}
